...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def assert_extra_args(
        ...
        if args.fp8_base_unet:
            args.fp8_base = (
                True  # if fp8_base_unet is enabled, fp8_base is also enabled for FLUX.1
            )
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        model_version_info, text_encoders, vae, diff_model = (
            flux_train_utils.load_target_model(args, accelerator, weight_dtype)
        )
        ...
        diff_model = convert_model_to_bf16(diff_model)

        return model_version_info, text_encoders, vae, diff_model
        ...
        if args.cache_text_encoder_outputs:
            if self.train_clip_l and not self.train_t5xxl:
                return text_encoders[
                    0:1
                ]  # only CLIP-L is needed for encoding because T5XXL is cached
            else:
            ...
            if len(diff_output_pr_indices) > 0:
                ...
                with torch.no_grad():
                    model_pred_prior = call_dit(
                        ...
                        guidance_vec=(
                            guidance_vec[diff_output_pr_indices]
                            if guidance_vec is not None
                            else None
                        ),
                        t5_attn_mask=(
                            t5_attn_mask[diff_output_pr_indices]
                ...
                model_pred_prior = flux_utils.unpack_latents(
    ...
    def prepare_text_encoder_fp8(
        ...
        if index == 0:  # CLIP-L
            logger.info(
                f"prepare CLIP-L for fp8: set to {te_weight_dtype}, set embeddings to {weight_dtype}"
            )
