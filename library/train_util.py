...
class ImageInfo:
    def __init__(
        ...
        self.text_encoder_outputs_npz: Optional[str] = (
            None  # set in cache_text_encoder_outputs
        )
    ...
    def process_caption(self, subset: BaseSubset, caption):
        ...
        if is_drop_out:
            ...
            if (
                ...
                caption = ", ".join(
                    c.strip() for c in caption.split(subset.keep_tokens_separator)
                )
    ...
    def __getitem__(self, index):
        ...
        def none_or_stack_elements(tensors_list, converter):
            ...
            if (
                ...
                return None
            return [
                torch.stack([converter(x[i]) for x in tensors_list])
...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            for img_path, caption, size in zip(img_paths, captions, sizes):
                info = ImageInfo(
                    img_path, num_repeats, caption, subset.is_reg, img_path
                )
                if size is not None:
...
def add_optimizer_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--lr_scheduler_type", type=str, default="", help="custom scheduler module"
    )
    parser.add_argument(
        "--lr_scheduler_args",
        type=str,
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        "--resume", type=str, default=None, help="saved state to resume training"
    )
    ...
    parser.add_argument(
        "--max_train_steps", type=int, default=1600, help="training steps"
    )
    parser.add_argument(
        "--max_train_epochs",
        type=int,
...
def sample_image_inference(
    ...
    logger.info(
        f"height: {height} width: {width} sample_steps: {sample_steps} scale: {scale} sample_sampler: {sampler_name}{f' seed: {seed}' if seed is not None else ''}"
    )
