...
class SAVEUS8bit(Optimizer8bit):
    r"""
    Implements the 8-bit SAVEUS optimization algorithm, incorporating techniques from ADOPT.
    ...
    Arguments:
        ...
        optim_bits (int, optional):
            Bit representation used for optimizer states (default: 8).
        percentile_clipping (int, optional):
            Percentile for histogram-based gradient clipping (default: 100).
        block_wise (bool, optional):
            Whether to use block-wise quantization (default: True).
    """
    ...
    def __init__(
        ...
        super(SAVEUS8bit, self).__init__(
            params,
            defaults,
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError("SAVEUS8bit does not support sparse gradients")
                ...
                if len(state) == 0:
                    state["step"] = 0
                    # Use 8-bit tensors for state
                    state["ema"] = bnb.nn.Params8bit(
                        torch.zeros_like(p.data, dtype=torch.float)
                    ).to(p.device)
                    state["ema_squared"] = bnb.nn.Params8bit(
                        torch.zeros_like(p.data, dtype=torch.float)
                    ).to(p.device)

                # Convert 8-bit state to float32 for computation
                ema = state["ema"].float_value
                ema_squared = state["ema_squared"].float_value
                ...
                # Update EMA of gradient (temporary float32 computation)
                ema.mul_(beta1).add_(grad, alpha=1 - beta1)
                # Amplify gradient with EMA
                grad.add_(ema, alpha=amp_fac)
                # Update EMA of squared gradient (temporary float32 computation)
                ema_squared.mul_(beta2).addcmul_(grad, grad, value=1 - beta2)

                # Store back to 8-bit state tensors
                state["ema"].data = ema
                state["ema_squared"].data = ema_squared
